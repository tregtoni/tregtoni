...
export default function FemijeFilterSidebar({
  slug, nenkategoria, cmimi_min, cmimi_max, qyteti, rendit,
  femijeFilters, clearUrl, hasFilters,
}: Props) {
  ...
  const mf = femijeFilters

  const isVeshje = nenkategoria === 'Veshje fëmijësh'
  const isKepuce = nenkategoria === 'Këpucë fëmijësh'
  const isLodra  = nenkategoria === 'Lodra'

  const madhesiaOpt = isVeshje ? MADHESIA_VESHJE : isKepuce ? MADHESIA_KEPUCE : null
        ...
        >
          <div style={{ padding: '16px' }}>
            ...
            <FilterSection title="Mosha">
              <ChipSelect name="mosha" options={MOSHA} value={mf.mosha} />
            </FilterSection>

            {/* ── GJINIA ── */}
            <FilterSection title="Gjinia">
              <ChipSelect name="gjinia" options={GJINIA} value={mf.gjinia} />
            </FilterSection>

            {/* ── MADHËSIA (veshje / këpucë) ── */}
            {madhesiaOpt && (
              <>
                <Divider />
                <FilterSection title="Madhësia">
                  <select name="madhesia" defaultValue={mf.madhesia ?? ''} style={inputSt}>
                    ...
                    {madhesiaOpt.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </FilterSection>
              </>
            )}
            ...
            {isLodra && (
              <>
                ...
                <FilterSection title="Lloji">
                  <ChipSelect name="femije_lloji" options={LLOJI_LODRA} value={mf.femije_lloji} />
                </FilterSection>
              </>
            )}
