        ...
        <div style={{ display: 'flex', gap: '6px', padding: '8px 12px', background: '#fafafa', overflowX: 'auto' }}>
          {images.map((url, i) => (
            <button
              key={i}
              onClick={() => setActive(i)}
              style={{ flexShrink: 0, width: '60px', height: '60px', border: i === active ? '2px solid #E24B4A' : '2px solid transparent', borderRadius: '6px', overflow: 'hidden', cursor: 'pointer', padding: 0, background: 'none' }}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
